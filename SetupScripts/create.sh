...
echo $nimbus
#Copy to zookeeper
gcloud compute scp zookeeper_setup.sh zookeeper:~/
#Setup zookeeper
gcloud compute --project "substantial-art-232723" ssh "zookeeper" --command "chmod +x zookeeper_setup.sh && ./zookeeper_setup.sh $zk"
...
#Nimbus setup
gcloud compute --project "substantial-art-232723" ssh "nimbus" --command "chmod +x storm_setup.sh && ./storm_setup.sh $zk $nimbus $1"
...
for i in `seq 1 $1`; do
	...
	#Slave setup
	gcloud compute --project "substantial-art-232723" ssh "slave$i" --command "chmod +x storm_setup.sh && ./storm_setup.sh $zk $nimbus $1"
done